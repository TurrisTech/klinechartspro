...
import { fetchArevValues, type ArevGeneration, type ArevPoint } from './api'
import { dropStore, storeFor, type ArevStore, type Range } from './store'
import { isArevIndicator, parseTemplateName, type ExtendData } from './templates'

// Keeps every AREV pane fed, the way indicators/controller.ts does for `S:` templates: per
// pane it watches the chart's own indicator list, and for each AREV template found fetches
...
// from the store). Much simpler than the server-indicator controller because there is
// nothing to resolve (a template IS a generation, there are no params) and nothing to
// subscribe (the rows are written by hand-run research scripts, not a live feed — new data
// appears when a script is re-run, and a reload or range change picks it up).
...
export interface ArevController {
  sync(panes: ChartProPane[]): void
}

export function createArevController(): ArevController {
  const wired = new Map<string, WiredPane>()

  const label = (b: Binding): string => {
    const base = b.generation.toUpperCase()
  ...
  const apply = (entry: WiredPane, b: Binding): void => {
    ...
    try {
      ...
      console.warn('[arev] override failed', err)
    }
  }
  ...
  const fetchGap = async (entry: WiredPane, b: Binding, gap: Range): Promise<void> => {
    ...
    try {
      ...
      while (from < gap.to && !b.disposed && pages < 20) {
        pages++
        const result = await fetchArevValues(
          `${b.vendor}:${b.ticker}`,
          b.interval,
          b.generation,
          from,
        ...
        const points: ArevPoint[] = result.points
      ...
      apply(entry, b)
      console.error('[arev] history fetch failed', err)
    } finally {
  ...
  const reconcile = (entry: WiredPane): void => {
    ...
    try {
      indicators = entry.chart.getIndicators().filter((i) => isArevIndicator(i.name))
    } catch {
      return
    }
    ...
    for (const ind of indicators) {
      ...
      const generation = parseTemplateName(ind.name)
      if (!generation) continue
      const storeKey = `${generation}|${vendor}:${symbol.ticker}|${interval}`
      const b: Binding = {
        indicatorId: ind.id,
        name: ind.name,
  ...
  // Debug hook, mirroring window.__wdIndicators: what each pane's AREV templates
