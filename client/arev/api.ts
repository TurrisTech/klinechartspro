...
// arev21 is arev19's model sampled at fresh price extremes instead of WMA crosses, and
// identical in everything else, so drawing the two together is a controlled read on the
// gate alone.
export const AREV_GENERATIONS = ['arev19', 'arev20', 'arev21'] as const
...
export interface ArevPoint {
  ...
  n: number
  /** P(price rises from here to the next sample): `(prediction / n + 1) / 2`. */
  p: number
   ...
   * lookback-bar extreme for arev21. The model is fitted only on such bars and has no
   * edge off one. The wire name is arev19's and is kept across generations.
   */
  atCross: boolean
