...
2. **Has the data had a chance to change?** `ChartLayer.staleAt(fetchedAt)` says when what is
   held can first differ from what the server would answer. Levels are computed on 1W and 1M,
   so a book can only change at a 17:00 market-day close — `levelsStaleAt` returns the next
   one. A layer that declares nothing falls back to a flat five-minute timer, which for levels
   meant a full refetch per pane twelve times an hour to be handed the same book.
...
`../levels/levels.test.ts` (the staleness horizon including both DST directions, what
identifies a request, what is drawn, and the tick case end to end). The controller's DOM half
has no coverage; the decisions it makes were moved out of it so they could have some.
