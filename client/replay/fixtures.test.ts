...
// server repo is reachable (the workspace layout, or SERVER_FIXTURES) the sync script's
// --check mode is run; elsewhere (a CI without the sibling repo) the check is skipped, and
// the fixtures' own provenance line is asserted instead.

const here = resolve(import.meta.dir, '../..')
const source = process.env.SERVER_FIXTURES ?? resolve(here, '../../wdashboard-server/main/tests/sim/fixtures')
...
describe('engine fixtures', () => {
  test('are identical to the server copy when it is reachable', () => {
    ...
    const proc = Bun.spawnSync(['bash', resolve(here, 'scripts/sync-engine-fixtures.sh'), '--check'], {
      env: { ...process.env, SERVER_FIXTURES: source }
    })
